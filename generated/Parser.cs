...
/*
  ...
  To summarize:
    ...
    * Symbol.Item6 -> memory pointer (for Structs. -1 when not in use.) See: http://lists.cs.uiuc.edu/pipermail/llvmdev/2010-November/035930.html for reasoning
    * Symbol.Item7 -> structname -> string name of stuct that the entry is an instantiation of. "not a struct" if not used.
*/
...
/*--------------------------------------------------------------------------*/
  ...
  public void Write (string filename) {
	...
	void ProcDecl() {
		...
		Expect(9);
		Ident(out name);
		currentScope.Push(new Symbol(name, (int)TastierKind.Proc, (int)TastierType.Undefined, openScopes.Count, -1,-1,"not a struct"));
	...
	void ConstDecl(bool external) {
		...
		sym = new Symbol(name,(int)TastierKind.Const,(int)type, 0,currentScope.Count(s => s.Item2 == (int)TastierKind.Var || s.Item2 == (int)TastierKind.Const),-1,"not a struct");
		...
		while (la.kind == 30) {
			...
			Type(out type);
			Ident(out name);
			sym = new Symbol(name, (int)TastierKind.Const,(int)type,0,currentScope.Count(s => s.Item2 == (int)TastierKind.Const || s.Item2 == (int)TastierKind.Var),-1,"not a struct");
			currentScope.Push(sym);
